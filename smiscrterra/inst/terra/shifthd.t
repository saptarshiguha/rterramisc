...
--------------------------------------------------------------------------------
local function bsHDVariance2(rng,  src,  nb,q,grain)   
   local wprecomp = ffi.gc( preComputeBetaDiff(#src,q),Rbase.free)
   grain = grain or 50
   local l = #src
...
function A.shifthd2 (x_,y_, nboot_,grain_)
   ...
   for i = 1,9 do
      local q = i/10
      local sex = bsHDVariance2( rng, x,  nboot[0],q,grain)
      local sey = bsHDVariance2( rng, y,  nboot[0],q,grain)
      local dif = A.hd(y.ptr, #y,q) - A.hd(x.ptr,#x,q)
      ret[i-1] = R.Robj{type='numeric', with = {dif-crit*math.sqrt(sex+sey), dif + crit*math.sqrt(sex+sey),dif}}
   end
   return ret
...
local terra bsHDVariance3(rng : &smisc.gsl.gsl_rng,  src:&double, srclength:int, nb:int,q:double,grain:double)   
...
function A.shifthd3 (x_,y_, nboot_,grain_)
   local x,y, nboot,grain = R.Robj(R.duplicateObject(x_)), R.Robj(R.duplicateObject(y_)), R.Robj(nboot_),R.Robj(grain_)[0]
   local crit,rng,ret = 80.1/math.pow(math.min(#x, #y),2) + 2.73, smisc.default_rng,R.Robj{type='vector', length = 9}
   local b  = terralib.new(F2, {rng, x.ptr, y.ptr,#x,#y,nboot[0], grain})
   local terra eval_for_q(index:int, input:&double, d:&F2)
      var sex = bsHDVariance3( d.rng, d.x, d.nx, d.nb, (index+1.0)/10.0 ,d.grain)
      var sey = bsHDVariance3( d.rng, d.y, d.ny, d.nb, (index+1.0)/10.0 ,d.grain)
